...
CREATE OR REPLACE FUNCTION public.fn_datos_publicos_candidato(ids uuid[])
 RETURNS TABLE(id uuid, codigo text, categoria categoria_candidato, edad integer, provincia text, altura_cm integer, peso_kg integer, fotos text[], talla_camisa text, anchura_pecho text, talla_pantalon text, anchura_cintura text, talla_calzado text, tipo_perfil jsonb, habilidades jsonb, carnes_conducir jsonb, idiomas_detalle jsonb, complexion text, tipo_pelo text, origen_etnia text, talla_chaqueta text, talla_zapato text)
 LANGUAGE sql
